/*
 *
 * SGLRender.java
 *
 * Created by Wuwang on 2016/10/15
 */
package com.dfsx.videoeditor.videorender;

import android.opengl.GLSurfaceView;
import android.view.View;

import com.dfsx.editengine.bean.VideoFrameBuffer;
import com.dfsx.videoeditor.videorender.filter.AFilter;
import com.dfsx.videoeditor.videorender.filter.ColorFilter;
import com.dfsx.videoeditor.videorender.filter.ContrastColorFilter;

import javax.microedition.khronos.egl.EGLConfig;
import javax.microedition.khronos.opengles.GL10;

/**
 * Description:
 */
public class SGLRender implements GLSurfaceView.Renderer {

    private AFilter mFilter;
    private VideoFrameBuffer mFrameBuffer;
    private int width, height;
    private boolean refreshFlag = false;
    private EGLConfig config;

    public SGLRender(View mView) {
        mFilter = new ContrastColorFilter(mView.getContext(), ColorFilter.Filter.NONE);
    }

    public void setFilter(AFilter filter) {
        refreshFlag = true;
        mFilter = filter;
        if (mFrameBuffer != null) {
            mFilter.setFrameBuffer(mFrameBuffer);
        }
    }

    public void refresh() {
        refreshFlag = true;
    }

    public AFilter getFilter() {
        return mFilter;
    }

    public void setFrameBuffer(VideoFrameBuffer buffer) {
        this.mFrameBuffer = buffer;
        mFilter.setFrameBuffer(buffer);
    }

    @Override
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        this.config = config;
        mFilter.onSurfaceCreated(gl, config);
    }

    @Override
    public void onSurfaceChanged(GL10 gl, int width, int height) {
        this.width = width;
        this.height = height;
        mFilter.onSurfaceChanged(gl, width, height);
    }

    @Override
    public void onDrawFrame(GL10 gl) {
        if (refreshFlag && width != 0 && height != 0) {
            mFilter.onSurfaceCreated(gl, config);
            mFilter.onSurfaceChanged(gl, width, height);
            refreshFlag = false;
        }
        mFilter.onDrawFrame(gl);
    }
}
