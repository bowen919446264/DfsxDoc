 ...
 */
package com.dfsx.videoeditor.videorender.filter;
...
import com.dfsx.videoeditor.gl.ShaderUtils;
...
public abstract class AFilter implements GLSurfaceView.Renderer {
    ...
    public void setFrameBuffer(VideoFrameBuffer frameBuffer) {
    ...
    public void onSurfaceChanged(GL10 gl, int width, int height) {
        ...
        int w = frameBuffer != null ? frameBuffer.getWidth() : 1920;
        int h = frameBuffer != null ? frameBuffer.getHeight() : 1080;
        float sWH = w / (float) h;
    ...
    private int createTexture() {
        ...
        if (frameBuffer != null && !frameBuffer.isEmpty()) {
            ...
            imageBuffer.put(frameBuffer.getBufferData());
            imageBuffer.position(0);
            GLES20.glTexImage2D(GLES20.GL_TEXTURE_2D, 0, getInternalFormat(frameBuffer.getConfig()), width, height, 0, getInternalFormat(frameBuffer.getConfig()),
                    getImageDataType(frameBuffer.getConfig()), imageBuffer);
            Log.i("Image", "textureId === " + texture[0]);
            return texture[0];
        }
        return 0;
    }

    private int getImageDataType(Bitmap.Config config) {
        int format;
        switch (config) {
            case ALPHA_8:
                ...
                break;
            case RGB_565:
                format = GLES20.GL_UNSIGNED_SHORT_5_6_5;
                break;
            case ARGB_4444:
                format = GLES20.GL_UNSIGNED_SHORT_4_4_4_4;
                break;
            case ARGB_8888:
                format = GLES20.GL_UNSIGNED_BYTE;
                break;
    ...
    private int getInternalFormat(Bitmap.Config config) {
        int format;
        switch (config) {
            case ALPHA_8:
