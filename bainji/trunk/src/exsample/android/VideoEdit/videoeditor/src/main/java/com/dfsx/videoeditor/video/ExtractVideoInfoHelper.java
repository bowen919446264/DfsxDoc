...
public class ExtractVideoInfoHelper {
    private MediaMetadataRetriever mMetadataRetriever;
    private long fileLength = 0;//毫秒

    public ExtractVideoInfoHelper(String path) {
     ...
     * @param timeMs 毫秒
     */
    public FrameData extractFrame(long timeMs) {
        //第一个参数是传入时间，只能是us(微秒)
        ...
        Bitmap bitmap = null;
        long realTime = 0L;
        for (long i = timeMs; i < fileLength; i += 20) {
        ...
        return new FrameData(realTime, bitmap);
    }

    public long getKeyFrameTime(long timeMs) {
        long realTime = 0L;
        for (long i = timeMs; i < fileLength; i += 20) {
            Bitmap bitmap = mMetadataRetriever.getFrameAtTime(i * 1000, MediaMetadataRetriever.OPTION_CLOSEST_SYNC);
            if (bitmap != null) {
                realTime = i;
                bitmap.recycle();
                return realTime;
    ...
    public String extractFrames(String OutPutFileDirPath, long timeMs) {
        FrameData frame = extractFrame(timeMs);
        return PictureUtils.saveImageToSD(frame.frameBitmap, OutPutFileDirPath);
    }
